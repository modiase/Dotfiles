...

{
  programs.fish = {
    ...
    functions = {
      gadd = "git add $argv";
      gamd = "git commit --amend --no-edit $argv";
      gblg = "git reflog show (git branch --show) $argv";
      gci = "git commit $argv";
      gco = "git checkout $argv";
      gcp = "git cherry-pick $argv";
      gdff = "git diff $argv";
      gfch = "git fetch $argv";
      ggc = "git gc $argv";
      ggrep = "git grep $argv";
      ggrph = "git log --all --decorate --oneline --graph $argv";
      ghm = "test \"(git branch --show)\" = \"(git symbolic-ref --short HEAD)\" or git checkout (git symbolic-ref --short HEAD) $argv";
      glg = "git log --oneline $argv";
      gls = "git ls-files $argv";
      gmb = "git merge-base $argv";
      gmrg = "git merge $argv";
      gpath = "git status &>/dev/null; and python -c 'import sys;assert len(sys.argv) >= 3, \"Missing argument\";from pathlib import Path;print(Path(sys.argv[2]).absolute().relative_to(Path(sys.argv[1])))' (git rev-parse --show-toplevel) $argv";
      gpll = "git pull $argv";
      gpsh = "git push $argv";
      gpwd = "git status &>/dev/null; and python -c 'import sys;from pathlib import Path;print(Path(sys.argv[2]).relative_to(Path(sys.argv[1])))' (git rev-parse --show-toplevel) (pwd) $argv";
      grb = "git rebase $argv";
      grbi = "git rebase -i (git merge-base HEAD (git symbolic-ref --short refs/remotes/origin/HEAD | cut -d '/' -f2)) $argv";
      grbm = "git rebase (git symbolic-ref --short refs/remotes/origin/HEAD | cut -d '/' -f2) $argv";
      grlg = "git reflog $argv";
      grm = "git rm $argv";
      grmt = "git remote $argv";
      grst = "git reset $argv";
      grsto = "git restore $argv";
      grt = "git rev-parse --show-toplevel &>/dev/null; and cd (git rev-parse --show-toplevel) or echo \"Not in a git repo\" $argv";
      grv = "git revert $argv";
      gsbtr = "git subtree $argv";
      gshow = "git show $argv";
      gst = "git status $argv";
      gstsh = "git stash $argv";
      gsw = "git switch $argv";
      gtag = "git tag $argv";
      gtch = "GIT_COMMITTER_DATE=\"(date)\" git commit --amend --no-edit --date \"(date)\" $argv";
      gupd = "git fetch origin main:main; and git rebase main $argv";
      gwhich = "git branch --show $argv";
      gwt = "git worktree $argv";
